...
These three questions are orthogonal and exhaustive. They are a reasoning lens the model is
prompted to apply within each turn — not a state machine enforced by the harness. The agent
iterates through tool calls until it has something worth keeping, then calls
`commit_changes(summary=…)` to flush staged writes and exit the run.
...
The system prompt (H4) defines the protocol: which tools are available, how `commit_changes` ends a run, how writes are staged before commit, what can be verified, and what must be rolled back.
